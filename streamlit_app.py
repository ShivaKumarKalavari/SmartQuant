...
index = pd.MultiIndex.from_product([all_dates, categories, warehouses],
                                   names=['date', 'product_category', 'warehouse_location'])
df_grouped = data.groupby(['date', 'product_category', 'warehouse_location'])['product_sales_quantity'].sum().reindex(
    index, fill_value=0).reset_index()

# Feature Engineering
df_grouped.sort_values(['product_category', 'warehouse_location', 'date'], inplace=True)
for lag in [1, 2, 3]:
    df_grouped[f'lag_{lag}'] = df_grouped.groupby(['product_category', 'warehouse_location'])[
        'product_sales_quantity'].shift(lag)

window_sizes = [3, 6]
for window in window_sizes:
    df_grouped[f'rolling_mean_{window}'] = df_grouped.groupby(['product_category', 'warehouse_location'])[
        'product_sales_quantity'].transform(lambda x: x.rolling(window, min_periods=1).mean())
    df_grouped[f'rolling_std_{window}'] = df_grouped.groupby(['product_category', 'warehouse_location'])[
        'product_sales_quantity'].transform(lambda x: x.rolling(window, min_periods=1).std())
...
df_grouped['time_idx'] = (df_grouped['date'].dt.year - df_grouped['date'].dt.year.min()) * 12 + (
            df_grouped['date'].dt.month - df_grouped['date'].dt.month.min())
...
selected_year_range = st.sidebar.slider('Select Year Range', int(data['year'].min()), int(data['year'].max()),
...
selected_location_pie = st.sidebar.selectbox("Select Warehouse Location for Market Share",
                                             data['warehouse_location'].unique(), key="market_share_location")
selected_year_range_pie = st.sidebar.slider("Select Year Range for Market Share", int(data['year'].min()),
                                            int(data['year'].max()),
                                            (int(data['year'].min()), int(data['year'].max())), key="market_share_year")
selected_month_range_pie = st.sidebar.slider("Select Month Range for Market Share", 1, 12, (1, 12),
                                             key="market_share_month")
...
st.subheader(
    f"Market Share Analysis for {selected_location_pie} ({selected_year_range_pie[0]} - {selected_year_range_pie[1]}, Months {selected_month_range_pie[0]} - {selected_month_range_pie[1]})")
...
if not data_filtered_pie.empty:
    ...
    if category_share.sum() > 0:
        ...
        ax_pie.pie(category_share, labels=category_share.index, autopct='%1.1f%%', startangle=90,
...
if st.sidebar.button('Predict Sales'):
    ...
    future_dates = pd.date_range(start=last_date + pd.DateOffset(months=1), end=f'{selected_year}-{selected_month}-01',
                                 freq='MS')
    ...
    for date in future_dates:
        features = {
            ...
            'rolling_std_3': np.std(current_sales[-3:]) if len(current_sales) >= 3 else np.std(current_sales) if len(
                current_sales) > 0 else 0,
            'rolling_mean_6': np.mean(current_sales[-6:]) if len(current_sales) >= 6 else np.mean(current_sales),
            'rolling_std_6': np.std(current_sales[-6:]) if len(current_sales) >= 6 else np.std(current_sales) if len(
                current_sales) > 0 else 0
        }
    ...
    st.write(
        f"The forecated sales quantity for the category '{selected_category}' in the location '{selected_location}' for the year '{selected_year}' and month '{selected_month}' is predicted around:",
        future_df.tail(1)['predicted_sales'].values[0])
    st.write("\n\n")
    st.write("If you want the 'Forecasted sales' upto the selected date :\n")
    st.write(future_df)
